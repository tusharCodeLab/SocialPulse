  ...
  return (
    <>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.05 }} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
        <MetricTile icon={Users} label="Followers" value={formatNum(igAccount?.followers_count || 0)} color="bg-[#E4405F]/10" />
        ...
        <MetricTile icon={Eye} label="Total Reach" value={formatNum(totals.totalReach)} color="bg-chart-reach/10" />
        <MetricTile icon={Image} label="Posts" value={String(posts.length)} color="bg-chart-impressions/10" />
      </motion.div>
              ...
              {hasData ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={postsTrend.map((v, i) => {
                    ...
                    return {
                      ...v,
                      engRate: reach > 0 ? (((post?.likes_count || 0) + (post?.comments_count || 0)) / reach * 100).toFixed(2) : 0,
                    };
                  })}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(222,30%,15%)" />
                    <XAxis dataKey="date" tick={{ fontSize: 10 }} stroke="hsl(215,20%,50%)" />
